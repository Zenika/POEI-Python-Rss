...
== Performance
Application should always update its UI in less than one second.

== Scalability
10 users in July
Each user read roughly 100 feeds
Each feed can be updated 50  times an hour

== Availability

Application must work all day long.
Application should worj 24 hours a day starting september

An RSS feed content must be shown to users less than 5 minutes after the RSS feed has been read.
No RSS entry content should be missed.
An RSS feed content must be shown to users less than one hour after having been published on original website.

== Security

The feed list a user is subscribed to is a confidential data according to GDPR
The item list a user have seen is a confidnetial data according  to GDPR


== Fault tolerance
We target a 99.99% availability level.
