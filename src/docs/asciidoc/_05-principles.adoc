...
* https://fr.wikipedia.org/wiki/Principe_KISS?useskin=vector[KISS]
* https://fr.wikipedia.org/wiki/YAGNI?useskin=vector[YAGNI]
* Keep stack easily deployable
* Read feed content on one side, and present user interface on another side
* If extensions are planned (as an example to read Facebook, Twitter, ..), prefer RSS as an intermediate format
